	...
	async function executePlugin(plugin2) {
		...
		const unresolvedPluginsForThisPlugin =
			((_a2 = plugin2.dependsOn) == null
				? void 0
				: _a2.filter(
						(name) => plugins2.some((p) => p._name === name) && !resolvedPlugins.includes(name)
					)) ?? []
...
function resolveUnrefHeadInput(ref2) {
	if (ref2 instanceof Promise || ref2 instanceof Date || ref2 instanceof RegExp) return ref2
...
const _routes = [
	{
		name: 'account',
		path: '/account',
		component: () => import('./_nuxt/account-CSIsKs8G.js')
	},
	{
		name: 'index',
		path: '/',
		component: () => import('./_nuxt/index-Db7vWHNc.js')
	}
]
...
const layouts = {
	default: defineAsyncComponent(() =>
		import('./_nuxt/default-CusM0wjH.js').then((m) => m.default || m)
	)
}
			...
			return h(
				...
				{ name: props.name, route: props.route, ...attrs },
				{
					default: (routeProps) => {
						...
						if (
							!nuxtApp.isHydrating &&
							!hasChildrenRoutes(forkRoute, routeProps.route, routeProps.Component) &&
							previousPageKey === key
						) {
...
const _sfc_main$1 = {
	...
	setup(__props) {
		...
		const stack = void 0
		const _Error404 = defineAsyncComponent(() => import('./_nuxt/error-404-CqKakS0D.js'))
		const _Error = defineAsyncComponent(() => import('./_nuxt/error-500-BghbsiOs.js'))
